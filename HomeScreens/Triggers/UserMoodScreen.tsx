...
const UserMoodScreen = () => {
    ...
    const getUserNotes =async()=>{
     await AsyncStorage.getItem('token').then((value) =>{
      ...
      fetch(Apilink+`/auth/getmoodtrackerdoc`, {
        method: "GET",
        headers: {
        ...
        .then((responseJson)=>{
          ...
          console.log(responseJson?.all_data)
        }).catch((error)=>{
          console.log(error);
        })
  ...
  useEffect(() => {
    ...
    const ItemView = ({ item }) => {
        console.log(item)
      return (
        <TouchableOpacity  onPress={() => navigation.navigate('UserMoodDetailScreen',{name:item?.username, moodTracker: item?.mood_tracker})}>
        <LinearGradient
  ...
  return (
    <ScrollView refreshControl={ <RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        <View>
          <Text style={{marginHorizontal:30,marginTop:10,marginBottom:5,fontFamily:'Poppins-Regular',fontSize:15}}>Users Moods</Text>
