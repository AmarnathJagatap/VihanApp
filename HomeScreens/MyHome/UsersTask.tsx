...
const UserTasks = () => {
  ...
  useEffect(() => {
    ...
    const ItemView = ({ item }) => {
        console.log(item)
      return (
        <TouchableOpacity  onPress={() => navigation.navigate('UserTaskDetailScreen',{name:item?.username, sessionNotes: item?.session_notes})}>
        <LinearGradient
    ...
    <ScrollView>
      ...
      <LinearGradient
        ...
        colors={['#FFFFFF','rgba(255, 255, 255, 8)','rgba(255, 255, 255, 5) @ 100%']} style={styles.button}>
                  <Text style={styles.buttonText}>Users Tasks</Text>
                  
      </LinearGradient>
      </TouchableOpacity>
    </LinearGradient>
        <View>
          <Text style={{marginHorizontal:30,marginTop:10,marginBottom:5,fontFamily:'Poppins-Regular',fontSize:15}}>Give & View tasks of Users</Text>
