...
const UserJournal = () => {
    ...
    const getUserNotes =async()=>{
     await AsyncStorage.getItem('token').then((value) =>{
      ...
      fetch(Apilink+`/auth/getjourneydoc`, {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    ...
    const ItemView = ({ item }) => {
        console.log(item)
      return (
        <TouchableOpacity  onPress={() => navigation.navigate('UsersJournalDetailScreen',{name:item?.username, myJourney: item?.my_journey})}>
        <LinearGradient
  ...
  return (
    <ScrollView refreshControl={ <RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        <View>
          <Text style={{marginHorizontal:30,marginTop:10,marginBottom:5,fontFamily:'Poppins-Regular',fontSize:15}}>Users Journals</Text>
